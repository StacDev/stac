...
### New Features
- Added **Stac Agent Skills** for enhanced agent-based interactions
- Enhanced **input decoration** with new border options
- Added **copyWith support to textTheme** in both `stac` and `stac_core` packages
- **Re-exported stac_core** through the stac package for simpler imports
- **Open-sourced Stac CLI** with release workflow

### Refactor
- Migrated linting from `flutter_lints` to `lints/recommended` and updated dev dependencies
- Converted `stac_logger` to a pure Dart package
- Ignored `pubspec_overrides.yaml` files

### Documentation
- Added skills documentation
- Improved package READMEs with quick start guides and detailed features
